<?php
	class order extends DController {
    	...
    	public function order_details($order_code){
			...
			$ordermodel = $this->load->model('ordermodel');
			$cond = "$table_product.id_product = $table_order.product_id AND $table_order.order_code = '$order_code'";
			$cond_info = "$table_order.order_code = '$order_code'";
			$data['order_details'] = $ordermodel->list_order_details($table_product,$table_order,$cond);
    	...
    	public function order_confirm($order_code){
			...
			if($result==1){
				...
				$message['msg']="Xử lí đơn hàng thất bại thất bại";
				header('Location:'.BASE_URL.'/order?msg='.urldecode(serialize($message)));
			}
    	}
	} 
